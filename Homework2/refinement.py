...
class Refine:
    ...
    def execute_action(self):
        ...
        if len(action)==2 and type(action) is tuple:
            help.execute_pick_action(action[0], action[1])
        elif len(action)==3 and type(action) is tuple:
            help.execute_place_action(action[0], action[1], action[2])
        else:
            help.execute_move_action(action)
    ...
    def refine_plan(self, plan_file, planner):
        ...
        """
        action_list = []
       # Read the plan file to generate corresponding actions
        h = Helper()
        dirpath = os.path.dirname(__file__)
        rel_path_file = os.path.join(dirpath, '../objects.json')
        with open(rel_path_file) as json_file:
        ...
        with open(plan_file, "r") as pf:
            ...
            if k[0] == 'move':
                goal_location = []
                obj=k[3][0:k[3].index('_', k[3].index('_') + 1)]
                if 'book' in obj:
                    loc = objects['books'][obj].get('load_loc')
                else:
                    loc= objects['bins'][obj].get('load_loc')
                for i in loc:
                    goal_location.append((float(i[0]), float(i[1])))
                act, current, flag = self.get_path(init_state, goal_location)
                temp=act
                init_state = current
            ...
            while plan:
                ...
                if k[0] == 'move':
                    ...
                    temp1 = (k[1], init_state)
                    action_list.append(temp1)
                elif k[0] == 'place':
                    temp1 = (k[1], k[3], init_state)
                    action_list.append(temp1)
